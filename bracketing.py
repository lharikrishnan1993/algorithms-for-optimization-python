...
class AforO_Bracketing:
    ...
    def run(self):
        ...
        univariate_function = lambda x: x**2 - 3.5*x + 2.5
        initial_start = 5.0
        bracket = self.bracketer.bracket_minimum(univariate_function, initial_start)
        st.write(f"For the example univariate function, $x^2 - 3.5x + 2.5$, the resulting bracket is **{bracket}**")

        fig = go.Figure()
        fig.add_trace(go.Scatter(x=[i for i in range(-15, 16)], y=[univariate_function(i) for i in range(-15, 16)], mode='lines', name='Univariate Function'))
        fig.add_trace(go.Scatter(x=[x for x in bracket], y=[univariate_function(x) for x in bracket], mode='markers', name='Intervals'))
